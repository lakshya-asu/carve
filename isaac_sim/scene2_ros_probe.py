...
class Scene2RosProbe:
    ...
    def __init__(self, topics: Ros2TopicNames | None = None) -> None:
        ...
        self.trajectory_pub = self.node.create_publisher(JointTrajectory, self.topics.joint_trajectory, 10)
    ...
    def publish_trajectory(
        ...
        message = self._JointTrajectory()
        message.joint_names = list(JOINT_NAMES)
        start = self._JointTrajectoryPoint()
        start.positions = list(start_positions)
        start.time_from_start.sec = 0
        start.time_from_start.nanosec = 0
        target = self._JointTrajectoryPoint()
        target.positions = list(target_positions)
        target.time_from_start.sec = int(duration_s)
        target.time_from_start.nanosec = int(round((duration_s - int(duration_s)) * 1_000_000_000))
        message.points = [start, target]
        self.trajectory_pub.publish(message)
        self.trajectories_published += 1
