...
if ($existingIsaac.Count -gt 0) {
    throw "Solution A requires a clean Isaac state; found $($existingIsaac.Count) existing Isaac process(es)."
}
if (-not $OutputRoot) {
...
try {
    ...
    if (-not $metrics.passed -and -not $AllowExpectedFailure) { throw "Solution A metrics reported failure: $($metrics.error)" }
    if ($Scenario -eq "nominal" -and $metrics.passed) {
