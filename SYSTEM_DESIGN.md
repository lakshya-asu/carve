...
The ROS 2 command and sensor contract is in `configs/ros2_interface.yaml`. The tested core-message bridge is in `isaac_sim/scene2_ros_bridge.py`. The final in-simulator task runner does not require an external motion process. It uses Isaac Sim Lula inverse kinematics, Cartesian TCP segments, joint-space quintic interpolation where appropriate, per-command position clamps, velocity and acceleration gates, collision-clear approach phases, and the actual articulation controller. MoveIt remains a replaceable external planner boundary through a future standard `FollowJointTrajectory` action bridge.
