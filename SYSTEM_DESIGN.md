...
## Scene 2.0 implemented baseline

`SCENE_DESIGN_V2.md` records the selection process. The implemented baseline uses the FANUC M-10iD/12 official-description reference as an eight-degree-of-freedom Isaac Sim articulation: J1 through J6 plus two prismatic gripper jaws. It has replaced the abstract Cartesian articulation in the final integrated A and B demonstrations.

The rebuilt cell uses a stationary cutter-entry tray and a side-pedestal robot layout. The sensor set includes one fixed overhead RGBD camera, one buffer RGBD mount for Solution B, a virtual presentation camera, conveyor state, an entry photoeye, robot joint feedback, bilateral jaw contact, and PLC-style cutter and safety I/O. A wrist camera is not required in the baseline.

The official robot description provides a stronger kinematic and visual basis than the earlier proxy. It does not establish OEM controller dynamics, certified collision fidelity, the exact food-grade surface package, washdown performance, food safety, or production readiness.
...
The robot command path records the actual six-joint articulation samples in `robot_joint_trajectory.json`. The document uses standard JointTrajectory joint names, positions, velocities, and simulation-relative timestamps. `scene2_ros_bridge.py` also accepts `trajectory_msgs/msg/JointTrajectory`, validates exact joint order, monotonic time, finite values, and limits, then samples the trajectory using the Isaac clock. It exposes a standard `FollowJointTrajectory` action when `control_msgs` is present. The action checks start, path, goal, and goal-time tolerances against measured articulation state. A runtime-neutral state machine covers success, tolerance failure, and cancellation.
...
The Scene 2.0 gripper is now an actuated part of the FANUC articulation. It adds two prismatic jaw joints named `finger_left` and `finger_right`. Each jaw has collision geometry, a contact sensor, a 0.25 m/s velocity limit, a 1.5 m/s2 acceleration metadata limit, a 70 N finite drive limit, and a 6,250 N/m series-compliance reference. The compact tool has a 220 mm clear opening, 140 mm pad depth, 100 mm pad coverage, and a grasp center 350 mm from the flange. The opening covers the current 200 mm maximum recipe width with 20 mm total clearance. Larger products require a different removable pad set or tool. The normal-force setpoint is 50 N per jaw. The 0.16 mm/N compliance and friction values are uncalibrated simulation assumptions.
...
The focused Scene 2.0 gate proves stage construction, six-axis controller execution, imported limits, bounded velocity and acceleration commands, rendered RGB and depth, stage save, and independent reload. A second gate, `validate_scene2_ros.ps1`, runs the same articulation and overhead RGBD sensor through ROS 2 Humble DDS. Isaac publishes `/clock`, measured J1 through J6 state, RGB, depth, and camera calibration. It subscribes to a strict six-joint command topic and applies accepted positions through the actual articulation controller.

The ROS 2 command and sensor contract is in `configs/ros2_interface.yaml`. The bridge is in `isaac_sim/scene2_ros_bridge.py`. The final in-simulator task runner does not require an external motion process. It uses Isaac Sim Lula inverse kinematics, Cartesian TCP segments, joint-space quintic interpolation where appropriate, per-command position clamps, velocity and acceleration gates, collision-clear approach phases, and the actual articulation controller.

The external planning workspace is under `ros2_ws`. It contains the FANUC model wrapper, SRDF, KDL inverse kinematics, OMPL RRTConnect, conservative joint limits, the MoveIt simple-controller mapping, fixed collision objects for the cell, and a timed pose client. The client rejects a requested arrival time that would shorten MoveIt's safe time-parameterized trajectory. It can stretch a plan to arrive later, then executes it through the standard action. Static contract tests pass. A live build and execution test is still blocked because ROS 2, MoveIt, `control_msgs`, and colcon are absent from the existing WSL environment.
...
The stationary-belt pickup gate uses Isaac Sim Lula inverse kinematics to generate top-down pregrasp, grasp, lift, transport, release, and retract poses. The workpiece is visible from the first recorded frame. It remains kinematic on the stationary belt until bilateral contact and becomes dynamic before lift. The gate requires at least 5 mm pad clearance, no unexpected contact pair, at least 100 mm lift, no more than 20 mm tool-relative drift, visible release, no workpiece teleport after recording starts, and no joint-limit violation.

`isaac_sim/run_scene2_integrated.py` now executes the complete task sequence on this arm. Rendered overhead RGB and metric depth pass through the YOLO26 interface, planar pose recovery, tracking, and prediction. The controller times a moving-conveyor intercept, requires bilateral contact, switches the product from its fixed-step conveyor fixture to dynamic PhysX motion, lifts and transports it without product pose writes, aligns it to `cut_target_frame`, releases it into the stationary tray, verifies delivery, acknowledges the result, and retracts. Solution B additionally releases on the buffer, captures a new rendered RGBD observation, corrects the observed pose change, regrips, and feeds the tray.
