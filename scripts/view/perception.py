...
Record to a file:
    ...
    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl \
        python scripts/view/perception.py --out perception.mp4 --seconds 20

Or watch it live in a window, which needs a display:

    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl DISPLAY=:0 \
        python scripts/view/perception.py --live --seconds 120
...
from robotics.core.frames import CameraPose, Intrinsics, world_to_pixel
...
from robotics.perception.pose_estimation import PerceptionRejectedError, estimate_from_depth, grip_axis_rad
from robotics.perception.tracking import BeltTracker
...
def _require_gui() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "  python scripts/view/perception.py --out perception.mp4 --seconds 30\n"
