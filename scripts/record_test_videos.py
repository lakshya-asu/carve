...
checks first did not hold. The saw videos come from scripts/view_saw.py; this
script records the arm reach, the grippers on their own, and each grasp on a leg.

Usage:
    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl python scripts/record_test_videos.py
"""
...
from meat_cell_sim.arm import UnreachableError, solve_ik
from meat_cell_sim.arms import ArmModel
from meat_cell_sim.cell import Cell
from meat_cell_sim.gripper import GRIPPER_SPECS, GripperModel, _finger_joint_addresses
from meat_cell_sim.product import LegConfig
from meat_cell_sim.scene import ASSETS, CellConfig
from meat_cell_sim.skills import AcquireShank, AcquireTrotterEnd, SelectShankGrasp, SelectTrotterEndGrasp
from skill_library import SUCCESS, Skill, SkillResult, run_skill
...
def film_gripper(gripper: GripperModel, path: Path) -> None:
    ...
    spec.attach(mujoco.MjSpec.from_file(str(ASSETS / GRIPPER_SPECS[gripper].asset)), prefix="g_", site=mount)
...
def main() -> None:
    ...
    for noisy in ("meat_cell_sim", "skill_library"):
