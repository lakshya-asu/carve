...
frame against the simulator's silhouette (extra pixels red, missed pixels blue).
    ...
    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl python scripts/view_edge_effects.py
...
from meat_cell_sim.product import leg_population
...
def main() -> None:
    ...
    with Cell(config, spec) as cell:
        ...
        references = [_with_depth(empty, sense_depth(empty.depth_m, GEMINI_335L, rng, edges=edges)) for _ in range(10)]
    ...
    mask = segmenter.segment(_with_depth(frame, edged)).mask
