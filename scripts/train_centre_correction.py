...
Validation on the last 10 training legs.
    ...
    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl python scripts/train_centre_correction.py
...
from meat_cell_sim.learned_centre_of_gravity import CentreCorrectionNet, correction_inputs
from meat_cell_sim.leg_segmentation import EmptyBeltReference, LegSegmenter
from meat_cell_sim.product import leg_half_width_m, leg_population
    ...
    for leg_index, leg in enumerate(leg_population(legs, seed=1)):
        ...
        with Cell(config, spec) as cell:
            ...
            for _ in range(frames_per_leg):
                ...
                noisy = _with_depth(frame, sense_depth(frame.depth_m, GEMINI_335L, rng, edges=cameras[camera]))
