...
Usage:
    env -u PYTHONPATH PYTHONPATH=src MUJOCO_GL=egl python scripts/measure_arm_reach.py
"""
...
from meat_cell_sim.arm import UnreachableError, arm_qpos_indices, solve_ik
    ...
    for arm_model in (ArmModel.UR20, ArmModel.SR20IA):
        with Cell(CellConfig(belt_speed_mps=0.0, arm=arm_model)) as cell:
            ...
            for label, height in HEIGHTS_ABOVE_BELT_M.items():
                ...
                print(f"| {arm_model.value} | {label} ({1000 * height:.0f} mm) | {reached} | {xs.size * ys.size} | {span} |")
