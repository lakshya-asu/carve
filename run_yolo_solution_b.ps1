...
try {
    & $isaacPython isaac_sim\run_cell.py `
        ...
        --output-root results\yolo\solution_b `
        --headless
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    $metrics = Get-Content results\yolo\solution_b\isaac_b\metrics.json -Raw | ConvertFrom-Json
    if (-not $metrics.passed) { throw "YOLO Solution B metrics reported failure" }
}
finally {
    Pop-Location
}
