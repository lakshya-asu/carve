param(
    ...
    [string]$Perturbation,
    [int]$Seed = 4601,
    [double]$BeltSpeedMps = 0.14,
    [double]$StartYM = -0.02,
    [double]$StartYawDeg = 25.0,
    [Parameter(Mandatory = $true)]
    [string]$OutputRoot
)
...
& "$PSScriptRoot\run_solution_a.ps1" `
    -Seed $Seed `
    -Scenario nominal `
...
if ($LASTEXITCODE -ne 0 -and -not $allowBaselineFailure) { throw "Solution D A reactive process failed" }
...
if ($metrics.passed -and $metrics.grasp.affordance.mode -ne "geometric") {
    throw "Solution D A did not retain the geometric paired-comparison selector"
}
if ($metrics.reactive_interception -and $metrics.reactive_interception.controller -ne $Controller) {
    throw "Solution D A controller evidence does not match the requested controller"
}
if ($Controller -eq "reactive" -and (-not $metrics.passed -or $metrics.reactive_interception.updates.Count -lt 1)) {
    throw "Solution D A produced no reactive update decision"
}
