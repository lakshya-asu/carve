...
Every integrated `return_to_idle` path now updates the presentation state to `idle` before final evidence is recorded. A focused regression test checks all seven call sites. Four focused tests passed. The affected seed 2703 rerun passed at `results/overnight/20260827_080654_solution_a_stale_observation_seed2703_v2`. Its final frame now shows `STATE IDLE` and `PLC READY`, and its machine-readable recovery audit passes.

The final complete command was `run_tests.ps1`. It passed with 151 Python tests and zero failures. Fresh evidence is `results/full_suite/20260827_081125156`. Setup, Scene 2, ROS command handling, the compliant gripper gate, nominal Solution A and B, both fail-closed audits, stage reload, rendered RGBD, YOLO26 segmentation, tracking, prediction, Lula IK, FANUC articulation control, bilateral PhysX contact, physical lift and retention, cutter-frame delivery, PLC acknowledgement, event traces, and saved H.264 media all passed.

Solution A recorded 11.444 mm position error, 0.002741 rad angle error, 25.000 ms delivery timing error, 7.508 ms interception timing error, 1,999 articulation commands, and 177.919 mm lift. Solution B recorded 21.140 mm position error, 0.002598 rad angle error, 16.667 ms delivery timing error, 7.886 ms interception timing error, 6.578 mm buffer sensor-to-oracle error, 3,521 articulation commands, and 177.931 mm lift. Both had zero unexpected contacts, zero motion-limit violations, zero product pose writes after grasp, matching stage and video hashes, acknowledged delivery, and a final visible `idle` state. Solution B retained the expected slip-correction evidence.

The corrected stale-observation check recorded two rendered observations, a 0.1 m/s track, 276 zero-motion articulation commands, no contact or lift as expected for a pre-commit rejection, no PLC acknowledgement, zero motion-limit violations, a matching reloaded USD, and 28 H.264 frames. Its event trace contains 10 monotonic records and terminates with `recovered` and `interception_stale`. The overhead depth array contains 307,200 finite positive values. Visual inspection confirmed the rendered RGB scene, aligned segmentation mask, stale-plan rejection, and safe return to `idle`.

Known joystick, deprecation, material, muted USD diagnostic, DLSS, render-variable, host-copy, and transform-history warnings remain. Final logs contained zero Isaac error lines and zero PhysX failure lines. No threshold changed. No Isaac or Kit process remained after any batch. `PRODUCT_RECIPES.md` remained untouched. T015 and the external ROS2 and MoveIt item in T016 remain blocked as previously documented. The machine record is `results/overnight/20260827_080654_solution_a_stale_observation_seed2703_v2/automation_summary.json`. The next dedicated check should alternate to Solution B with seed 2704, preferably `slip_correction`.
