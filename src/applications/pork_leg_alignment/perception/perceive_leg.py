...
Kept apart from `grasping/` because this needs the camera stack, and the policies must run in
...
# Only pixels standing this far above the belt shape the leg: the segmenter's own cut. Along the
# outline a stereo camera reports the belt's depth, or a blend, for some leg pixels; traced back
# along their rays those land far from the leg (up to 0.7 m on the Gemini model with edge effects),
# and one such pixel at each end doubled the perceived length.
STANDING_M = 0.010
# Pixels ignored at each end of the leg when finding its length: about 5 mm of a trotter tip at the
# Gemini 335L's 1.5 mm per pixel on the belt.
END_STRAY_PIXELS = 50
...
def perceive_leg(
    ...
    standing = height_m > STANDING_M
    if standing.sum() < 3:
        raise ValueError("fewer than 3 masked pixels stand above the belt")
    points, height_m = points[standing], height_m[standing]
    ...
    # A fixed number of pixels off each end, not a percentage: a leg tapers to a point at the trotter,
    # where trimming 0.5 percent of its pixels took 40 mm off its length, while the few blended edge
    # pixels that survive the standing filter added 36 mm when the ends were the plain extremes.
    trim = min(END_STRAY_PIXELS, (along_m.size - 1) // 2)
    ordered = np.partition(along_m, (trim, along_m.size - 1 - trim))
    ham_end_m, trotter_end_m = float(ordered[trim]), float(ordered[along_m.size - 1 - trim])
