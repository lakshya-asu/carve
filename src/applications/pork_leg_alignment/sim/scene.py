...
from robotics.hardware.cameras import DepthCameraModel, add_depth_camera
from robotics.hardware.grippers import GRIPPER_ASSETS, GRIPPER_SPECS, GripperModel
...
def leg_centre_y_for_rail_gap(leg: LegConfig, yaw_deg: float, gap_m: float) -> float:
    """Outline-centre y that leaves `gap_m` between a leg lying at `yaw_deg` and the far rail.

    The leg's reach toward the rail is half its length times cos(yaw) plus its widest half width
    times |sin(yaw)|: the extent of its bounding rectangle for yaw within 90 degrees of zero. The
    measurement scripts use it to put every leg in the population the same distance from the rail.
    """
    widest_m = float(leg_half_width_m(leg, np.linspace(0.0, 1.0, 101)).max())
    reach_m = 0.5 * leg.length_m * math.cos(math.radians(yaw_deg)) + widest_m * abs(math.sin(math.radians(yaw_deg)))
    return FAR_RAIL_INNER_Y_M - gap_m - reach_m
...
def build_spec(config: CellConfig | None = None) -> mujoco.MjSpec:
    ...
    gripper = mujoco.MjSpec.from_file(str(GRIPPER_ASSETS / GRIPPER_SPECS[config.gripper].asset))
