...
from applications.pork_leg_alignment.sim.product import LegConfig, leg_half_width_m
from applications.pork_leg_alignment.sim.scene import (
    FAR_RAIL_INNER_Y_M,
    CellConfig,
    SlabConfig,
    build_model,
    leg_centre_y_for_rail_gap,
    save_xml,
)
...
def test_rail_gap_placement_reaches_the_rail_by_length_square_and_by_width_turned() -> None:
    leg = LegConfig(length_m=0.70)
    widest_m = float(leg_half_width_m(leg, np.linspace(0.0, 1.0, 101)).max())
    assert leg_centre_y_for_rail_gap(leg, 0.0, 0.05) == pytest.approx(FAR_RAIL_INNER_Y_M - 0.05 - 0.35)
    assert leg_centre_y_for_rail_gap(leg, 90.0, 0.05) == pytest.approx(FAR_RAIL_INNER_Y_M - 0.05 - widest_m)
    assert leg_centre_y_for_rail_gap(leg, 0.0, 0.10) == pytest.approx(leg_centre_y_for_rail_gap(leg, 0.0, 0.05) - 0.05)
